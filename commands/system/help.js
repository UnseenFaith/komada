  ...
  return new Promise(resolve => {
    ...
    client.commands.forEach(command => {
      mps.push(new Promise(res => {
       ...
       .then(() => {
         let cat = command.help.category.slice(command.help.category.indexOf("commands/")+9).split("/")[0];
         let subcat = command.help.category.slice(command.help.category.indexOf("commands/")+9).split("/")[1];
         if (!cat) cat = "General";
