exports.run = async (client, msg, [type, name]) => {
	...
	switch (type) {
		case 'function':
			try {
				m = await msg.sendMessage(`Attempting to reload function ${name}`);
		...
		case 'inhibitor':
			try {
				m = await msg.sendMessage(`Attempting to reload inhibitor ${name}`);
		...
		case 'monitor':
			try {
				m = await msg.sendMessage(`Attempting to reload monitor ${name}`);
		...
		case 'provider':
			try {
				m = await msg.sendMessage(`Attempting to reload provider ${name}`);
		...
		case 'event':
			try {
				m = await msg.sendMessage(`Attempting to reload event ${name}`);
		...
		case 'command':
			switch (name) {
				...
				default:
					try {
						m = await msg.sendMessage(`Attempting to reload command ${name}`);
