...
module.exports = async (client) => {
  ...
  if (client.outBaseDir &&
    ...
    await loadCommandInhibitors(client, client.outBaseDir).catch(err => client.emit("error", client.funcs.newError(err)));
  }

  const langCounts = [];
  client.commandInhibitors.forEach((i) => {
  ...
  client.funcs.log(`Loaded ${client.commandInhibitors.size} command inhibitors${countMsg}.`);
};
