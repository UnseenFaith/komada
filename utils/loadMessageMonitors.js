...
module.exports = async (client) => {
  ...
  if (client.outBaseDir &&
    ...
    await loadMessageMonitors(client, client.outBaseDir).catch(err => client.emit("error", client.funcs.newError(err)));
  }

  const langCounts = [];
  client.messageMonitors.forEach((m) => {
  ...
  client.funcs.log(`Loaded ${client.messageMonitors.size} command monitors${countMsg}.`);
};
