...
exports.run = (client, msg, cmd) =>
   new Promise((resolve, reject) => {
     ...
     let args = msg.content.slice(prefixLength).split(" ").slice(1).join(" ")
     ...
     (function validateArgs(i) {
       ...
       if (currentUsage.type === "optional" && (args[i] === undefined || args[i] === "")) { // Handle if args length < required usage length
         ...
         reject(currentUsage.possibles.length === 1 ? `${currentUsage.possibles[0].name} is a required argument.` : `Missing a required option: (${currentUsage.possibles.map(p => p.name).join(", ")})`);
         ...
         switch (currentUsage.possibles[0].type) {
           case "literal":
             if (args[i].toLowerCase() === currentUsage.possibles[0].name.toLowerCase()) {
               ...
               reject(`Your option did not litterally match the only possibility: (${currentUsage.possibles.map(p => p.name).join(", ")})\nThis is likely caused by a mistake in the usage string.`);
             }
             break;
           case "msg":
           case "message":
         ...
         (function multiPossibles(p) {
           if (validated) {
             ...
             if (currentUsage.type === "optional" && !repeat) {
               ...
               reject(`Your option didn't match any of the possibilities: (${currentUsage.possibles.map(p => p.name).join(", ")})`);
             }
             return;
           }
