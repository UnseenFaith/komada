...
class SchemaManager extends CacheManager {
  ...
  async add(key, options, force = true) {
    ...
    if ("min" in options && Number.isNaN(options.min)) throw "The option min must be a number.";
    if ("max" in options && Number.isNaN(options.max)) throw "The option max must be a number.";
